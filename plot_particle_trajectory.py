...
def plot_particle_trajectory(ntot, npar, w_dir, UNIT_DENSITY, UNIT_LENGTH, UNIT_VELOCITY):
    ...
    P = pr.ploadparticles(0, w_dir, datatype='dbl', ptype='CR')
    number = P.id[npar];

    for j in range(ntot+1):
        P = pr.ploadparticles(j, w_dir, datatype='dbl',ptype='CR') # Loading particle data : particles.00ns_ch00.flt
    ...
    D = pp.pload(ntot,w_dir, datatype='dbl') # Load fluid data.
