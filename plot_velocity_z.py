...
def plot_velocity_z(ns, w_dir, UNIT_DENSITY, UNIT_LENGTH, UNIT_VELOCITY, datatype, excl_axis = 3, point = 0.5):
    c=2.998E10
    ...
    if(ndim == 3):
        if(excl_axis == 3):
            zpoint = math.floor(D.vx3.T.shape[0]* point)
            Vz = D.vx3.T[zpoint, :, :] * UNIT_VELOCITY / c
        elif(excl_axis == 2):
            zpoint = math.floor(D.vx3.T.shape[1]* point)
            Vz = D.vx3.T[:,zpoint, :] * UNIT_VELOCITY / c
        elif(excl_axis == 1):
            zpoint = math.floor(D.vx3.T.shape[2]* point)
            Vz = D.vx3.T[:,:,zpoint] * UNIT_VELOCITY / c
        else:
            print("wrong excluded axis\n")
            return
