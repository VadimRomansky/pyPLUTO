...
def plot_velocity_z(ns, w_dir, UNIT_DENSITY, UNIT_LENGTH, UNIT_VELOCITY, datatype, file_name = 'velocity_z.png', excl_axis = 3, point = 0.5, aspect = 'equal', transponse = False):
    c=2.998E10
    ...
    if(transponse):
        im2 = ax.imshow(Vz.T, origin='upper', norm = colors.Normalize(vmin = minV, vmax = maxV), aspect=aspect,extent=[D.x2.min()*UNIT_LENGTH, D.x2.max()*UNIT_LENGTH, D.x1.min()*UNIT_LENGTH, D.x1.max()*UNIT_LENGTH]) # plotting fluid data.
