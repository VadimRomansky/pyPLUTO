...
def plot_pressure_1d_series(number, w_dir, UNIT_DENSITY, UNIT_LENGTH, UNIT_VELOCITY, datatype, axis = 1, point1 = 0.5, point2 = 0.5):
    ...
    if (axis == 1):
        ...
        dx = (xmax - xmin) / Prs1.shape[1]
        x = dx * range(Prs1.shape[1]) + xmin
    elif (axis == 3):
        xmin = D.x3.min() * UNIT_LENGTH
        xmax = D.x3.max() * UNIT_LENGTH
        dx = (xmax - xmin) / Prs1.shape[2]
        x = dx * range(Prs1.shape[2]) + xmin
    else:
        print("wrong axis")
        return
