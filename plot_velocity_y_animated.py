...
def plot_velocity_y_animated(ntot, w_dir, UNIT_DENSITY, UNIT_LENGTH, UNIT_VELOCITY, datatype, file_name = 'velocity_y.gif', excl_axis = 3, point = 0.5, aspect = 'equal'):
    c = 2.998E10
    ...
    for i in range(ntot + 1):
        D = pp.pload(i, varNames = ['vx2'], w_dir = w_dir, datatype=datatype)  # Load fluid data.
        Vy = getScalarArray(D.vx2, UNIT_VELOCITY/c, excl_axis, point)
        if(np.amin(Vy) < minV):
            minV = np.amin(Vy)
        if(np.amax(Vy) > maxV):
    ...
    def update(frame_number):
        ...
        D = pp.pload(frame_number, varNames = ['vx2'], w_dir = w_dir, datatype=datatype)  # Load fluid data.
        Vy = getScalarArray(D.vx2, UNIT_VELOCITY/c, excl_axis, point)

        im2 = ax.imshow(Vy, origin='upper', norm=colors.Normalize(vmin=minV, vmax=maxV), aspect = aspect,
