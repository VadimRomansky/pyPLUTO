...
def plot_energy(ntot, w_dir, UNIT_DENSITY, UNIT_LENGTH, UNIT_VELOCITY, datatype):
    ...
    for n in range(ntot+1):
        ...
        for i in range(D.vx1.shape[0]):
            ...
            if(ndim > 1):
                for j in range(D.vx1.shape[1]):
                    ...
                    if(ndim > 2):
                        for k in range(D.vx1.shape[2]):
                            ...
                            Etherm[n] = Etherm[n] + (1.0/(gam-1))*(D.prs[i][j][k]*mu/D.rho[i][j][k])*dVx*dVy*dVz
                        ...
                        Etherm[n] = Etherm[n] + (1.0/(gam-1.0))*(D.prs[i][j]*mu/D.rho[i][j])*dVx*dVy
                ...
                Ekin[n] = Ekin[n] + D.rho[i] * UNIT_DENSITY * (gamma) * dVx
                Etherm[n] = Etherm[n] + (1.0/(gam-1.0))*(D.prs[i]*mu/D.rho[i])*dVx
    ...
    ax.legend(['full enrgy', 'kinetic energy', 'thermal energy', 'magnetic energy'])
    ax.set_yscale("log")
