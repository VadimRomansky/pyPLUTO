...
def plot_pressure_window(ns, w_dir, UNIT_DENSITY, UNIT_LENGTH, UNIT_VELOCITY, xmin, xmax, ymin, ymax, datatype, file_name = 'pressure_window.png', excl_axis = 3, point = 0.5, aspect = 'equal', transponse = False):
    ...
    if(transponse):
        im2 = ax.imshow(Prs.T, origin='upper', norm = colors.LogNorm(vmin = minPrs, vmax = maxPrs), aspect=aspect,extent=[D.x2.min()*UNIT_LENGTH, D.x2.max()*UNIT_LENGTH, D.x1.min()*UNIT_LENGTH, D.x1.max()*UNIT_LENGTH]) # plotting fluid data.
