...
def plot_By_2d(number, w_dir, UNIT_DENSITY, UNIT_LENGTH, UNIT_VELOCITY):
    ...
    V = np.abs(D.Bx2.T*np.sqrt(4*np.pi*UNIT_DENSITY*UNIT_VELOCITY*UNIT_VELOCITY))
    minB = np.amin(V)
    maxB = np.amax(V)
