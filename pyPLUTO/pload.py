...
class pload(object):
	def __init__(self, ns, varNames = None, w_dir=None, datatype=None, level = 0, x1range=None, x2range=None, x3range=None):
#		"""
#		Loads the data.
#		**Inputs**:
#			ns -- Step Number of the data file\n
#			varNames -- names of variablws to read\n
...
#		"""
		...
		if(varNames is None):
			...
			Data_dictionary = self.ReadDataVariableFile(self.NStepStr, varNames)
...
#		"""
	...
	def ReadDataVariableFile(self, num, varName):
#		"""Reads the data file generated from PLUTO code.
#		**Inputs**:
#			num -- Data file number in form of an Integer.
#		**Outputs**:
...
#		"""
		...
		if self.filetype == "single_file":
			datafilename = self.wdir+"data."+nstr+dataext
			self.ReadSingleFile(datafilename, varName, self.n1, self.n2,
								self.n3, endian, dtype, ddict)
		elif self.filetype == "multiple_files":
			self.ReadMultipleFiles(nstr, dataext, varName, self.n1, self.n2,
									self.n3, endian, dtype, ddict)
		else:
			print("Wrong file type : CHECK pluto.ini for file type.")
			print("Only supported are .dbl, .flt, .vtk, .hdf5")
