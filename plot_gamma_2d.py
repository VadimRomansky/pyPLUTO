...
def plot_gamma_2d(ns, w_dir, UNIT_DENSITY, UNIT_LENGTH, UNIT_VELOCITY):
    ...
    for i in range(nx):
        for j in range(ny):
            V[i][j] = np.log10(1.0/np.sqrt(1.0 - (D.vx1.T[i][j]**2 - D.vx2.T[i][j]**2 - D.vx3.T[i][j]**2)*(UNIT_VELOCITY*UNIT_VELOCITY)/9E20))
    ratio = 0.1 #from 0.5
    V1=V[int((0.5 - ratio)*nx):int((0.5 + ratio)*nx),int((0.5 - ratio)*ny):int((0.5 + ratio)*ny)]
    print(np.amax(V1))
    im2 = ax.imshow(V1, origin='upper',extent=[ratio*D.x1.min()*UNIT_LENGTH, ratio*D.x1.max()*UNIT_LENGTH, ratio*D.x2.min()*UNIT_LENGTH, ratio*D.x2.max()*UNIT_LENGTH]) # plotting fluid data.
