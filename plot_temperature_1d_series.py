...
def plot_temperature_1d_series(number, w_dir, UNIT_DENSITY, UNIT_LENGTH, UNIT_VELOCITY):
    plt.rcParams.update({'font.size': 15})
    plt.rcParams['text.usetex'] = True

    D = pp.pload(number, varNames = ['T'], w_dir = w_dir, datatype='dbl') # Load fluid data.
    ndim = len((D.T.shape))
    ...
    if (ndim == 1):
        T3 = D.T[:]
        D = pp.pload(int(number / 2), varNames=['T'], w_dir=w_dir, datatype='dbl')
        T2 = D.T[:]
        D = pp.pload(1, varNames=['T'], w_dir=w_dir, datatype='dbl')
        T1 = D.T[:]
    if (ndim == 2):
