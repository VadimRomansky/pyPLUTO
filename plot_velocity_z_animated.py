...
def plot_velocity_z_animated(ntot, w_dir, UNIT_DENSITY, UNIT_LENGTH, UNIT_VELOCITY, datatype, file_name = 'velocity_z.gif', excl_axis = 3, point = 0.5, aspect = 'equal', transponse = False, out_dir = ""):
    c = 2.998E10
    ...
    def update(frame_number):
        ...
        im2 = ax.imshow(Vz, origin='upper', norm=colors.Normalize(vmin=minV, vmax=maxV), aspect = aspect,
                        extent=[xmin, xmax, ymin, ymax])  # plotting fluid data.
        if(transponse):
            #np.flip(Vz, 0)
            im2 = ax.imshow(Vz.T, origin='lower', norm=colors.Normalize(vmin=minV, vmax=maxV), aspect=aspect,
